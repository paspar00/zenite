...
const MigrationNotice = ({stripeData}: { stripeData: StripeConnectAccountsResponse }) => {
    ...
    // Only show if Orbita Sports user has CA account but no completed IE account
    if (!isOrbitaTickets() || !caAccount || (ieAccount && ieAccount.is_setup_complete)) {
        return null;
    }
            ...
            <Group gap="md" mb="md" align="flex-start">
                ...
                <div style={{flex: 1}}>
                    ...
                    <Text size="xs" c="dimmed" fs="italic">
                        {t`Thanks for your support as we continue to grow and improve Orbita Sports!`}
                    </Text>
                </div>
            </Group>
        </Card>
    ...
    return (
        <div className={paymentClasses.stripeInfo}>
            ...
            <Text size="sm" c="dimmed" mb="lg">
                {getConfig("VITE_APP_NAME", "Orbita Sports")} charges platform fees to maintain and improve our services.
...
// Orbita Sports Cloud Multi-Platform Component
const OrbitaTicketsConnectStatus = ({account}: { account: Account }) => {
    ...
    // For new Orbita Sports users with no CA platform (either new or only IE)
...
const ConnectStatus = ({account}: { account: Account }) => {
    if (isOrbitaTickets()) {
        return <OrbitaTicketsConnectStatus account={account}/>;
    } else {
...
const PaymentSettings = () => {
    ...
    const vatSettingQuery = useGetAccountVatSetting(
        accountQuery.data?.id || 0,
        {
            enabled: !!accountQuery.data?.id && isOrbitaTickets()
        }
    );
    ...
    // Only for Orbita Sports Cloud - open-source doesn't have VAT handling
    useEffect(() => {
        if (typeof window === 'undefined') return;
    ...
    return (
        <>
            {isOrbitaTickets() && accountQuery.data && (
                <VatSettingsModal
            ...
            {/* Migration Notice - Show at the top for Orbita Sports users who need to migrate */}
            {isOrbitaTickets() && stripeAccountsQuery.data && <MigrationNotice stripeData={stripeAccountsQuery.data}/>}
